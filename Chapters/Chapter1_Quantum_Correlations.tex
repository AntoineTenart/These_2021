...
To keep things simple, we will take $\phi(t)$ uniformly distributed on $[0,2\pi]$ and assume that the phase ``jumps'' from a value to another with a time-independent probability. This is the so called wave packet model. Between two times $t$ and $t+dt$, the probability for the phase to change writes:
...
\noindent We take $\phi_1(t)$ and $\phi_2(t)$ to be uncorrelated random variables described by the same probability law characterized by the coherence $\tau_c$. the numerator of the first-order correlation function:
...
\noindent Since $s(\omega)$ is real, we can rewrite: \NOTE{A FAIRE}
...
\begin{equation}
    E(\bm{r}) \propto \int_{S} a(\bm{s}) e^{\frac{i \pi}{\lambda L}|\bm{r}-\bm{s}|^{2}} d \bm{s}
    \label{eq:amp_HBT}
\end{equation}
...
\noindent All the elementary emitters are incoherent and thus each have a random phase value, determined by an uniform probability law defined on the interval $[0,2\pi]$. They thus form an ensemble of \textbf{independent} random variables with the \textbf{same statistics}. We can therefore apply the Central Limit Theorem to the sum of their contributions $A(\bm{r})$ and find that it follows Gaussian statistics \cite{goodman2007speckle}. The fact that $A$ is a Gaussian variable allows us to simplify the four-term correlator into:
...
\noindent Since the source is incoherent, $\left\langle a^{*}(\bm{s}_1) a(\bm{s}_2)\right\rangle = I(\bm{s}_1) \delta_{\bm{s}_1,\bm{s}_2}$, giving
...
\noindent The state thus writes as a superposition of number states with the same number of photons in modes 1 and 2. The populations in modes 1 and 2 are therefore perfectly correlated. If we now look at the reduced state corresponding to mode 1 for instance, the reduced density matrix writes:
...
This inequality can be rewritten with creation/annihilation operators to work with two-body correlation functions. We introduce the notation $G^{(2)}_{i,j} = \mean{\hat{a}_i^{\dagger} \hat{a}_j^{\dagger} \hat{a}_i \hat{a}_j}$. The Cauchy-Schwarz inequality becomes \cite{kheruntsyan2012violation,walls2008}:

\begin{equation}
    G^{(2)}_{1,2} \leq \sqrt{G^{(2)}_{1,1} G^{(2)}_{2,2} }
\end{equation}

In the symmetrical case with $\mean{\hat{a}^{\dagger}_1 \hat{a}_1}=\mean{\hat{a}^{\dagger}_2 \hat{a}_2}$ (true for the non-degenerate parametric amplifier), we obtain $G^{(2)}_{1,1}=G^{(2)}_{2,2}$ and finally:
...
\begin{equation}
    \gtwo_A (1,2) > 2 \Longleftrightarrow \gtwo_A (1,2) > \gtwo_N (1,2)
\end{equation}
